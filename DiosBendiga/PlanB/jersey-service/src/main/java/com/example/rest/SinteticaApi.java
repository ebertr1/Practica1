...
public class SinteticaApi {
    ...
    public Response save(HashMap map) {
        ...
        try {
            ...
            // Lista para almacenar los parámetros faltantes
            StringBuilder missingParams = new StringBuilder();
            
            // Verifica qué parámetros faltan
            if (map.get("nombre") == null) {
                missingParams.append("nombre, ");
            }
            if (map.get("direccion") == null) {
                missingParams.append("direccion, ");
            }
            if (map.get("telefono") == null) {
                missingParams.append("telefono, ");
            }
            if (map.get("latitud") == null) {
                missingParams.append("latitud, ");
            }
            if (map.get("longitud") == null) {
                missingParams.append("longitud, ");
            }
            if (map.get("horario") == null) {
                missingParams.append("horario, ");
            }
    
            // Si faltan parámetros, se incluye la lista de parámetros faltantes
            if (missingParams.length() > 0) {
                String missing = missingParams.toString();
                // Eliminar la última coma y espacio
                missing = missing.substring(0, missing.length() - 2);
    
                res.put("msg", "Error");
                res.put("data", "Faltan los siguientes parámetros: " + missing);
